...
from swagger_client.models.cipher_suite import CipherSuite  # noqa: F401,E501
from swagger_client.models.protocol_version import ProtocolVersion  # noqa: F401,E501
...
class HttpServiceProperties(object):
    ...
    def __init__(self, session_timeout=None, certificate=None, redirect_host='', protocol_versions=None, cipher_suites=None, connection_timeout=None):  # noqa: E501
    ...
    def session_timeout(self, session_timeout):
        ...
        :type: int
        """
        if session_timeout is not None and session_timeout > 2147483647:  # noqa: E501
            raise ValueError("Invalid value for `session_timeout`, must be a value less than or equal to `2147483647`")  # noqa: E501
        if session_timeout is not None and session_timeout < 0:  # noqa: E501
            raise ValueError("Invalid value for `session_timeout`, must be a value greater than or equal to `0`")  # noqa: E501
    ...
    def redirect_host(self, redirect_host):
        ...
        :type: str
        """
        if redirect_host is not None and not re.search('^(?=.{1,255}$)[0-9A-Za-z](?:(?:[0-9A-Za-z]|-){0,61}[0-9A-Za-z])?(?:\\.[0-9A-Za-z](?:(?:[0-9A-Za-z]|-){0,61}[0-9A-Za-z])?)*\\.?$|^$', redirect_host):  # noqa: E501
            raise ValueError("Invalid value for `redirect_host`, must be a follow pattern or equal to `/^(?=.{1,255}$)[0-9A-Za-z](?:(?:[0-9A-Za-z]|-){0,61}[0-9A-Za-z])?(?:\\.[0-9A-Za-z](?:(?:[0-9A-Za-z]|-){0,61}[0-9A-Za-z])?)*\\.?$|^$/`")  # noqa: E501
